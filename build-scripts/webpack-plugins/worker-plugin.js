// This is build output of master at July 10, 2020
// b68e6b502c59639aedbf5279f1fc83698483ac24
// While waiting for a new release with webpack 5 compat
// This doesn't work yet -> TypeError: ParserHelpers.addParsedVariableToModule is not a function
// https://github.com/GoogleChromeLabs/worker-plugin/pull/84
...
WorkerPlugin.prototype.apply = function apply(compiler) {
  ...
  compiler.hooks.normalModuleFactory.tap(NAME, function (factory) {
    ...
    var parse = function (parser, esModule) {
      var handleWorker = function (workerTypeString) {
        return function (expr) {
          ...
          if (!dep.isString()) {
            parser.state.module.warnings.push({
              message:
                "new " +
                workerTypeString +
                '("..", { type: "module" }) will only be bundled if passed a String.',
            });
            return false;
          }
          ...
          var isStrictModule =
            ...
            (parser.state.buildMeta &&
              parser.state.buildMeta.strictHarmonyModule); // Querystring-encoded loader prefix (faster/cleaner than JSON parameters):

          var loaderRequest =
            workerLoader +
            "?name=" +
            encodeURIComponent(opts.name || workerId) +
            (isStrictModule ? "&esModule" : "") +
            "!" +
            dep.string; // Unique ID for the worker URL variable:
          ...
          if (isStrictModule) {
            ...
            if (!HarmonyImportSpecifierDependency) {
              throw Error(
                NAME +
                  ": Failed to import HarmonyImportSpecifierDependency. This plugin requires Webpack version 4."
              );
            } // This is essentially the internals of "prepend an import to the module":

            var dependency = new HarmonyImportSpecifierDependency(
              loaderRequest,
              module,
          ...
          if (this$1.options.workerType) {
            ParserHelpers.toConstantDependency(
              parser,
              JSON.stringify(this$1.options.workerType)
